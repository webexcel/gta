...
function fetch_all(testid) {
    ...
    $.ajax({
        url: "prepareTest.php?getTest=" + testid + "&take=" + take, success: function (result) {
            $('#pWait').hide();
...
function mark_start() {
    $.ajax({
        url: "prepareTest.php?start=" + tid, success: function (result) {
...
function f(stdid, testid, qid, stdans) {
...
function testOver() {
    ...
    $.ajax({
        url: "prepareTest.php?over=" + tid + "&ssid=" + ssid, success: function (result) {

            document.getElementById('container').innerHTML = "<div class='alert alert-info text-center alert-rounded'>" + result + "</div><p id='res_box' ><input class='btn btn-' style='margin:auto;background: #ee0f0f; border-color:ee0f0f;color: white;' type='button' value='Close' onclick='submitTest();' ></p>";
